...
function eventsManager(client, io) {
    const worldChatManager = WorldChatManager(client, io);
    const gameRoomManager = GameRoomManager(client, io);
    const { joinWorldChat, leaveWorldChat, getWorldChatPlayers, broadcastMessage } = worldChatManager;
    const { registerClient, removeClient, createGameRoom, joinGameRoom, leaveRoom, handleSetPlayerReady, handleChipPlacement, handleInGameChat } = gameRoomManager;
    /// WORLD CHAT
